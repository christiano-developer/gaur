...
const AIPatrolPage = () => {
  ...
  const handleStartService = async (serviceName: string) => {
    ...
    try {
      const response = await apiClient.post(
        `/api/v1/ai/patrol/services/${serviceName}/start`,
      );
      if (response.success) {
  ...
  const handleStopService = async (serviceName: string) => {
    ...
    try {
      const response = await apiClient.post(
        `/api/v1/ai/patrol/services/${serviceName}/stop`,
      );
      if (response.success) {
        await loadPatrolData();
      }
  ...
  const getRiskBadge = (riskLevel: string) => {
    ...
    return (
      <Badge
        variant={variants[riskLevel as keyof typeof variants] || "outline"}
      >
        {riskLevel}
      </Badge>
    );
  ...
  return (
    <div className="space-y-6 p-2 bg-green-800 min-h-screen justify-center flex items-center">
      <div>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Central command for AI-powered fraud detection and social media
              monitoring
            </p>
          </div>
          <div className="flex gap-2">
            <Button
              onClick={handleStartPatrolSession}
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {patrolStats?.system_status.services_running || 0} /{" "}
                    {patrolStats?.system_status.total_services || 0}
                  </p>
                </div>
              ...
              <Progress
                value={
                  ((patrolStats?.system_status.services_running || 0) /
                    (patrolStats?.system_status.total_services || 1)) *
                  100
                }
                className="mt-2"
              />
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {liveThreats.filter((t) => t.risk_level === "HIGH").length} high
                risk
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {patrolStats?.system_status.active_sessions || 0}
                  </p>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Current status of AI patrol services
                  </CardDescription>
                </CardHeader>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Latest AI-detected fraud alerts
                  </CardDescription>
                </CardHeader>
                    ...
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-sm text-muted-foreground">
                            {threat.source_platform}
                          </span>
                        </div>
                        <p className="text-sm">
                          {threat.content_text.substring(0, 100)}...
                        </p>
                  ...
                  <CardHeader>
                    ...
                    <CardDescription>
                      {name === "facebook_scraper" &&
                        "Monitors Facebook groups for fraud content"}
                      {name === "telegram_scraper" &&
                        "Monitors Telegram channels for suspicious messages"}
                      {name === "ai_analyzer" &&
                        "AI-powered content analysis and fraud detection"}
                      {name === "domain_monitor" &&
          ...
          <TabsContent value="threats" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Real-time fraud detection alerts from social media monitoring
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        {threat.detected_keywords.length > 0 && (
                          <>
                            ...
                            <span>
                              Keywords:{" "}
                              {threat.detected_keywords.slice(0, 3).join(", ")}
                            </span>
                            {threat.detected_keywords.length > 3 && (
                              <span>
                                +{threat.detected_keywords.length - 3} more
                              </span>
                            )}
                          </>
                        )}
                  ...
                  {liveThreats.length === 0 && (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        The AI patrol system is monitoring for fraud patterns
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Fraud detection performance overview
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-red-600">
                        {
                          liveThreats.filter((t) => t.risk_level === "HIGH")
                            .length
                        }
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-yellow-600">
                        {
                          liveThreats.filter((t) => t.risk_level === "MEDIUM")
                            .length
                        }
                      </span>
                    </div>
